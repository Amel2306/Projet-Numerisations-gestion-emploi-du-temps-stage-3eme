...
class Moment {
    ...
    giveActivite(parc) {
      ...
      for (const [activite, dispo] of this.activite_dispo) {
        let found = false;
        for (const  act of parc) { // vérifie que l'activité n'apparait pas déjà dans le parcours donné
          if (act !== null && act === activite) {
            found = true;
            break;
          } 
        }
        if (!found) { // si elle n'apparait pas déjà
          activitePrise = activite;
          const valeurActuelle = this.activite_dispo.get(activitePrise); 
          if (valeurActuelle !== undefined && valeurActuelle >= this.nb_eleve_max) { // on regard qu'elle puisse bien prendre le nombre d'élèves minimum pour un parcours
            this.activite_dispo.set(activitePrise, valeurActuelle - this.nb_eleve_max); // on lui enlève le nombre d'élève que l'activité va occuper pour ce moment
          }
